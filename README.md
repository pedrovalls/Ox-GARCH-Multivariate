...
$\bf{H}_{t}$=$\bf{C}^{\prime }\bf{C}$+$\sum\limits_{i=1}^{p}\bf{A}_{i}^{\prime }(\bf{\epsilon }_{t-i}\bf{\epsilon }_{t-i}^{\prime})\hbf{A}_{i}$+$\sum\limits_{j=1}^{q}\mathbf{B}_{j}^{\prime}\mathbf{H}_{t-j}\mathbf{B}_{j}$
